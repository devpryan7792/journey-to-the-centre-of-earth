...
// PURPOSE: Loads the de_humani_corporis_fabrica.glb book model, places it at the
...
export function BookClosure() {
  ...
  // Load the book model
  const { scene: bookScene } = useGLTF('/models/de_humani_corporis_fabrica.glb');
  
  // Clone the scene to avoid shared state
  const bookModel = useMemo(() => {
    const clone = bookScene.clone(true);
    
    // Apply warm material tinting to all meshes
    clone.traverse((child) => {
      if (child.isMesh) {
        // Store original material for cover identification
        if (child.material) {
          child.material = child.material.clone();
          // Warm tint: shift all materials toward espresso/gold palette
          child.material.color = child.material.color || new THREE.Color('#f2e6c9');
          child.material.emissive = new THREE.Color('#c9a84c');
...
// Preload the model
useGLTF.preload('/models/de_humani_corporis_fabrica.glb');
