...
function EffectsPipeline({ degraded }) {
  ...
  // 1. Cinematic Atmospherics: Heavy Vignette & Subtle Film Grain Noise
  const vignetteDarkness = 1.0; // heavy
  const noiseOpacity = 0.15; // subtle
...
export function MainCanvas() {
  ...
  return (
    ...
    >
      ...
      {/* Adaptive Performance: drops DPR when FPS drops, but we no longer aggressively disable all effects to preserve the visual identity */}
      <PerformanceMonitor
        onIncline={handleIncline}
        onDecline={handleDecline}
        flipflops={3}
        bounds={(refreshrate) => [30, Math.min(60, refreshrate)]}
